...
#define CSV_NOMBRE_VALIDO             "ejemplos/pok_ok.csv"          // 3 válidas
#define CSV_VACIO                     "ejemplos/vacio.csv"       // 0 líneas
#define CSV_LARGO                     "ejemplos/largo.csv"       // N grande
#define CSV_NORMAL                    "ejemplos/normal.csv"      // csv chico y correcto
#define CSV_MUCHOS                    "ejemplos/pok_muchos.csv"      // stress
#define CSV_NO_EXISTE                 "ejemplos/no_existe.csv"        // no debe existir
#define CSV_NULO                      NULL

#define CSV_MEZCLA                    "ejemplos/pok_mixto.csv"       // válidas + inválidas
...
bool recolectar_ids(struct pokemon *p, void *extra)
{
    ...
    if (r->cantidad_ids < r->capacidad) r->ids[r->cantidad_ids++] = p->id;
...
bool contar_si_esta_en_b(struct pokemon *p, void *extra)
{
    ...
    if (tp1_buscar_id(ctx->b, p->id)) ctx->c++;
    return true;
}
...
void tp1_leer_archivo_nombre_valido()
{
    ...
    if (tp) {
        pa2m_afirmar(tp1_cantidad(tp) == ESP_OK_CANTIDAD,
                     "Cantidad de pokemons insertados correctamente (%d)", ESP_OK_CANTIDAD);
        tp1_destruir(tp);
    }
}
...
void tp1_leer_archivo_archivo_vacio_crea_pokedex_vacia()
{
    ...
    if (tp) {
        pa2m_afirmar(tp1_cantidad(tp) == ESP_VACIO_CANTIDAD,
...
void tp1_leer_archivo_archivo_largo_crea_correctamente()
{
    ...
    pa2m_afirmar(tp != NULL, "Csv grande no dificulta creacion de estructura");
    if (tp) {
        pa2m_afirmar(tp1_cantidad(tp) == ESP_LARGO_CANTIDAD,
                     "Cuenta correcta de elementos en archivo largo (%d)", ESP_LARGO_CANTIDAD);
        tp1_destruir(tp);
    }
}
...
void tp1_leer_archivo_archivo_normal_crea_con_cantidad_correcta()
{
    ...
    pa2m_afirmar(tp != NULL, "CSV con cantidad normal de pokemones se crea correctamente");
    if (tp) {
        pa2m_afirmar(tp1_cantidad(tp) == ESP_NORMAL_CANTIDAD,
                     "Cantidad correcta en archivo normal (%d)", ESP_NORMAL_CANTIDAD);
        tp1_destruir(tp);
    }
}
...
void tp1_leer_archivo_muchos_pokemones_cuenta_bien()
{
    ...
    pa2m_afirmar(tp != NULL, "CSV de muchas lineas crea la estructura válida");
    if (tp) {
        pa2m_afirmar(tp1_cantidad(tp) == ESP_MUCHOS_CANTIDAD,
                     "Cantidad de pokemones esperados (%d)", ESP_MUCHOS_CANTIDAD);
        tp1_destruir(tp);
    }
}
...
void tp1_leer_archivo_mezcla_validos_invalidos()
{
    ...
    pa2m_afirmar(tp != NULL, "Csv con lineas validas como invalidas se crea correctamente");
    if (tp) {
        pa2m_afirmar(tp1_cantidad(tp) == ESP_MEZCLA_VALIDOS,
                     "Cantidad de líneas válidas correctamente (%d)", ESP_MEZCLA_VALIDOS);
        tp1_destruir(tp);
    }
}
...
void tp1_leer_archivo_todas_lineas_invalidas()
{
    ...
    pa2m_afirmar(tp != NULL, "Csv totalmente invalido crea estructura correctamente");
    if (tp) {
        pa2m_afirmar(tp1_cantidad(tp) == ESP_TODAS_INVALIDAS_CANT,
                     "Cantidad 0 cuando todas las líneas son inválidas (%d)",ESP_TODAS_INVALIDAS_CANT);
        tp1_destruir(tp);
    }
}
...
void tp1_leer_archivo_linea_final_sin_newline()
{
    ...
    pa2m_afirmar(tp != NULL, "Csv con ultima línea sin '\\n' se procesa correctamente");
    if (tp) {
        pa2m_afirmar(tp1_cantidad(tp) == ESP_SIN_NL_FINAL_CANT,
                     "Cuenta correcta incluyendo la última línea sin newline (%d)", ESP_SIN_NL_FINAL_CANT);
        tp1_destruir(tp);
    }
}
...
{
    tp1_t *tp = tp1_leer_archivo(CSV_NOMBRE_VALIDO);
    pa2m_afirmar(tp != NULL, "Csv válido devuelve estructura no-NULL");
    if (!tp) return;

    size_t cant = tp1_cantidad(tp);
    pa2m_afirmar(cant == ESP_OK_CANTIDAD,
                 "tp1_cantidad devuelve cantidad correcta para CSV válido (%d)", ESP_OK_CANTIDAD);
...
{
    tp1_t *tp = tp1_leer_archivo(CSV_MEZCLA);
    pa2m_afirmar(tp != NULL, "CSV mixto (válidos + inválidos) crea estructura no-NULL");
    if (!tp) return;

    size_t cant = tp1_cantidad(tp);
    pa2m_afirmar(cant == ESP_MEZCLA_VALIDOS,
                 "tp1_cantidad cuenta solo las líneas válidas (%d)", ESP_MEZCLA_VALIDOS);
...
void tp1_cantidad_archivo_grande_coincide()
{
    ...
    size_t cant = tp1_cantidad(tp);
    pa2m_afirmar(cant == ESP_LARGO_CANTIDAD,
                 "tp1_cantidad cuenta cantidad correcta en archivo grande (%d)", ESP_LARGO_CANTIDAD);
...
{
    tp1_t *tp = tp1_leer_archivo(CSV_VACIO);
    pa2m_afirmar(tp != NULL, "Archivo existente pero vacío crea estructura no-NULL");
    if (!tp) return;

    size_t cant = tp1_cantidad(tp);
    pa2m_afirmar(cant == ESP_VACIO_CANTIDAD,
                 "tp1_cantidad cuenta cantidad 0 cuando el archivo está vacío");
...
{
    pa2m_afirmar(tp1_cantidad(NULL) == 0, "tp1_cantidad con null devuelve cero");
}
...
void tp1_union_mismo_tp_devuelve_otro_igual()
{
    ...
    pa2m_afirmar(u != NULL, "Unión(A, A) devuelve estructura válida");
    if (u) {
        size_t cant_u = tp1_cantidad(u);
        pa2m_afirmar(cant_u == cant_a, "La cantidad en la unión coincide con la original (%zu)", cant_a);

        struct pokemon *p;
        p = tp1_buscar_id(a, 1); if (p) pa2m_afirmar(tp1_buscar_id(u, 1) != NULL, "id=1 está en la unión");
        p = tp1_buscar_id(a, 2); if (p) pa2m_afirmar(tp1_buscar_id(u, 2) != NULL, "id=2 está en la unión");
        p = tp1_buscar_id(a, 3); if (p) pa2m_afirmar(tp1_buscar_id(u, 3) != NULL, "id=3 está en la unión");
		p = tp1_buscar_id(a, 4); if (p) pa2m_afirmar(tp1_buscar_id(u, 4) != NULL, "id=4 está en la unión");
		p = tp1_buscar_id(a, 5); if (p) pa2m_afirmar(tp1_buscar_id(u, 5) != NULL, "id=5 está en la unión");
...
{
    tp1_t *a = tp1_leer_archivo(CSV_NORMAL); /* {1,2,3} */
    tp1_t *b = tp1_leer_archivo(CSV_MEZCLA); /* {2,3,4} */
    ...
    pa2m_afirmar(u != NULL, "Unión(A, B) devuelve estructura válida");
    if (u) {
        pa2m_afirmar(tp1_cantidad(u) == 6,
                     "Unión sin repetidos tiene %d elementos", 6);
...
void tp1_union_dos_tp_vacio_devuelve_tp_vacio()
{
    ...
    pa2m_afirmar(u != NULL, "Unión de vacíos devuelve estructura válida");
    if (u) {
        pa2m_afirmar(tp1_cantidad(u) == ESP_VACIO_CANTIDAD, "Cantidad 0 en la unión vacía");
        tp1_destruir(u);
    }
...
{
    tp1_t *largo  = tp1_leer_archivo(CSV_LARGO);  /* 15 elementos (tu lista) */
    tp1_t *normal = tp1_leer_archivo(CSV_NORMAL); /* 10 elementos */
    pa2m_afirmar(largo && normal, "Tp A y B creados correctamente");
    if (!largo || !normal) { if (largo) tp1_destruir(largo); if (normal) tp1_destruir(normal); return; }

    tp1_t *u = tp1_union(largo, normal);
    pa2m_afirmar(u != NULL, "Unión(largo, normal) devuelve estructura válida");
    if (u) {
        pa2m_afirmar(tp1_cantidad(u) == 18,
                     "Cantidad correcta en la unión largo+normal (%d)", 18);

        pa2m_afirmar(tp1_buscar_id(u, 30) != NULL, "Esta id=30 (solo en largo)");
        pa2m_afirmar(tp1_buscar_id(u, 2) != NULL, "Esta id=2 (solo en normal)");
        pa2m_afirmar(tp1_buscar_id(u, 17) != NULL, "Esta id=17 (está en ambos)");
...
{
    tp1_t *a = tp1_leer_archivo(CSV_NORMAL); /* {1,2,3} */
    tp1_t *b = tp1_leer_archivo(CSV_NOMBRE_VALIDO); /* {2,3,4} */
    tp1_t *v = tp1_leer_archivo(CSV_VACIO);
    pa2m_afirmar(a && b && v, "Tp A, B y vacío creados correctamente");
    if (!a || !b || !v) {
		if (a)
			tp1_destruir(a);
		if (b)
    ...
    pa2m_afirmar(u1 != NULL, "Unión(A, B) válido");
    if (!u1) { tp1_destruir(a); tp1_destruir(b); tp1_destruir(v); return; }

    tp1_t *u2 = tp1_union(u1, a);
    pa2m_afirmar(u2 != NULL, "(A ∪ B) ∪ A válido");

    tp1_t *u3 = u2 ? tp1_union(u2, b) : NULL;
    pa2m_afirmar(u3 != NULL, "((A ∪ B) ∪ A) ∪ B válido");

    tp1_t *u4 = u3 ? tp1_union(u3, v) : NULL;
    pa2m_afirmar(u4 != NULL, "… ∪ vacío válido");

    if (u4) {
        pa2m_afirmar(tp1_cantidad(u4) == 8,
                     "Tras múltiples uniones, la cantidad sigue siendo %d", 8);
    ...
    if (u4) tp1_destruir(u4);
    if (u3) tp1_destruir(u3);
    if (u2) tp1_destruir(u2);
    if (u1) tp1_destruir(u1);
...
void tp1_union_primer_parametro_null_devuelve_null()
{
    ...
    pa2m_afirmar(b != NULL, "Se crea ""otro_tp"" correctamente");
    if (!b) return;

    tp1_t *u = tp1_union(NULL, b);
    pa2m_afirmar(u == NULL, "tp1_union con primer parametro NULL devuelve NULL");
...
void tp1_union_segundo_parametro_null_devuelve_null()
{
    ...
    pa2m_afirmar(a != NULL, "Se crea ""un_tp"" correctamente");
    if (!a) return;

    tp1_t *u = tp1_union(a, NULL);
    pa2m_afirmar(u == NULL, "tp1_union con segundo parametro NULL devuelve NULL");
...
void tp1_union_ambos_parametros_null_devuelve_null()
{
    pa2m_afirmar(tp1_union(NULL, NULL) == NULL,
                 "tp1_union con ambos parámetros NULL devuelve NULL");
}
...
void tp1_interseccion_mismo_tp_devuelve_otro_igual()
{
    ...
    pa2m_afirmar(i != NULL, "Interseccion(A, A) devuelve estructura válida");
    if (i) {
        size_t cant_i = tp1_cantidad(i);
        pa2m_afirmar(cant_i == cant_a, "La cantidad en la intersección coincide con la original (%zu)", cant_a);

        struct pokemon *p;
        p = tp1_buscar_id(a, 1); if (p) pa2m_afirmar(tp1_buscar_id(i, 1) != NULL, "id=1 está en la intersección");
        p = tp1_buscar_id(a, 2); if (p) pa2m_afirmar(tp1_buscar_id(i, 2) != NULL, "id=2 está en la intersección");
        p = tp1_buscar_id(a, 3); if (p) pa2m_afirmar(tp1_buscar_id(i, 3) != NULL, "id=3 está en la intersección");
        p = tp1_buscar_id(a, 4); if (p) pa2m_afirmar(tp1_buscar_id(i, 4) != NULL, "id=4 está en la intersección");
        p = tp1_buscar_id(a, 5); if (p) pa2m_afirmar(tp1_buscar_id(i, 5) != NULL, "id=5 está en la intersección");
...
void tp1_interseccion_tp_distintos_devuelve_otro_con_suma_correcta()
{
    tp1_t *a = tp1_leer_archivo(CSV_NORMAL);
    tp1_t *b = tp1_leer_archivo(CSV_MEZCLA);
    pa2m_afirmar(a && b, "Tp A y B creados correctamente");
    if (!a || !b) { if (a) tp1_destruir(a); if (b) tp1_destruir(b); return; }

    /* cantidad esperada dinámica = |A ∩ B| */
    ...
    pa2m_afirmar(i != NULL, "Interseccion(A, B) devuelve estructura válida");
    if (i) {
        pa2m_afirmar(tp1_cantidad(i) == esperado,
                     "Intersección tiene %zu elementos", esperado);

        /* Chequeos condicionales de ids (si están en ambos) */
        struct pokemon *pa, *pb;
        pa = tp1_buscar_id(a, 1); pb = tp1_buscar_id(b, 1);
        if (pa && pb) pa2m_afirmar(tp1_buscar_id(i, 1) != NULL, "Contiene id=1");
        pa = tp1_buscar_id(a, 2); pb = tp1_buscar_id(b, 2);
        if (pa && pb) pa2m_afirmar(tp1_buscar_id(i, 2) != NULL, "Contiene id=2");
        pa = tp1_buscar_id(a, 3); pb = tp1_buscar_id(b, 3);
        if (pa && pb) pa2m_afirmar(tp1_buscar_id(i, 3) != NULL, "Contiene id=3");
        pa = tp1_buscar_id(a, 4); pb = tp1_buscar_id(b, 4);
        if (pa && pb) pa2m_afirmar(tp1_buscar_id(i, 4) != NULL, "Contiene id=4");
        pa = tp1_buscar_id(a, 5); pb = tp1_buscar_id(b, 5);
        if (pa && pb) pa2m_afirmar(tp1_buscar_id(i, 5) != NULL, "Contiene id=5");
...
void tp1_interseccion_dos_tp_vacio_devuelve_tp_vacio()
{
    ...
    pa2m_afirmar(i != NULL, "Intersección de vacíos devuelve estructura válida");
    if (i) {
        pa2m_afirmar(tp1_cantidad(i) == 0, "Cantidad 0 en la intersección vacía");
        tp1_destruir(i);
    }
...
void tp1_interseccion_largo_suma_correctamente()
{
    ...
    pa2m_afirmar(i != NULL, "Intersección(largo, normal) devuelve estructura válida");
    if (i) {
        pa2m_afirmar(tp1_cantidad(i) == esperado,
                     "Cantidad correcta en la intersección largo+normal (%zu)", esperado);

        pa2m_afirmar(tp1_buscar_id(i, 30) == NULL, "No está id=30 (solo en largo)");
        pa2m_afirmar(tp1_buscar_id(i, 2)  == NULL, "No está id=2 (solo en normal)");
        pa2m_afirmar(tp1_buscar_id(i, 1) != NULL || esperado==0, "Si comparten, id=17 está en la intersección");
...
void tp1_interseccion_unir_muchas_veces_sigue_contando_bien()
{
    tp1_t *a = tp1_leer_archivo(CSV_NORMAL);
    tp1_t *b = tp1_leer_archivo(CSV_NOMBRE_VALIDO);
    tp1_t *v = tp1_leer_archivo(CSV_VACIO);
    pa2m_afirmar(a && b && v, "Tp A, B y vacío creados correctamente");
    if (!a || !b || !v) { if (a) tp1_destruir(a); if (b) tp1_destruir(b); if (v) tp1_destruir(v); return; }
    ...
    pa2m_afirmar(i1 != NULL, "Intersección(A, B) válido");
    if (!i1) { tp1_destruir(a); tp1_destruir(b); tp1_destruir(v); return; }

    tp1_t *i2 = tp1_interseccion(i1, a);
    pa2m_afirmar(i2 != NULL, "(A ∩ B) ∩ A válido");

    tp1_t *i3 = i2 ? tp1_interseccion(i2, b) : NULL;
    pa2m_afirmar(i3 != NULL, "((A ∩ B) ∩ A) ∩ B válido");

    /* No intersecto con vacío para mantener la misma cantidad (paralelo al test de unión) */
    if (i3) {
        pa2m_afirmar(tp1_cantidad(i3) == esperado,
                     "Tras múltiples intersecciones, la cantidad sigue siendo %zu", esperado);

        /* si hay intersección real, probá algunos ids */
        if (esperado > 0) {
            struct pokemon *pa = tp1_buscar_id(a, 1), *pb = tp1_buscar_id(b, 1);
            if (pa && pb) pa2m_afirmar(tp1_buscar_id(i3, 1) != NULL, "Tiene id=1");
        }
    }

    if (i3) tp1_destruir(i3);
    if (i2) tp1_destruir(i2);
    if (i1) tp1_destruir(i1);
    tp1_destruir(a);
    tp1_destruir(b);
    tp1_destruir(v);
}
...
void tp1_interseccion_primer_parametro_null_devuelve_null()
{
    ...
    pa2m_afirmar(u == NULL, "tp1_interseccion con primer parametro NULL devuelve NULL");
...
void tp1_interseccion_segundo_parametro_null_devuelve_null()
{
    ...
    pa2m_afirmar(u == NULL, "tp1_interseccion con segundo parametro NULL devuelve NULL");
...
{
    pa2m_afirmar(tp1_interseccion(NULL, NULL) == NULL,
                 "tp1_interseccion con ambos parámetros NULL devuelve NULL");
}
...
void pruebas_tp1_interseccion(){
	...
	tp1_interseccion_tp_distintos_devuelve_otro_con_suma_correcta();
	tp1_interseccion_dos_tp_vacio_devuelve_tp_vacio();
	tp1_interseccion_largo_suma_correctamente();
	tp1_interseccion_unir_muchas_veces_sigue_contando_bien();
...
void tp1_diferencia_mismo_tp_devuelve_otro_vacio()
{
    ...
    pa2m_afirmar(d != NULL, "Diferencia(A, A) devuelve estructura válida");
    if (d) {
        pa2m_afirmar(tp1_cantidad(d) == 0, "La diferencia A\\A es vacía (0)");
        tp1_destruir(d);
    }
    tp1_destruir(a);
}
...
void tp1_diferencia_tp_distintos_devuelve_el_mismo()
{
    ...
    pa2m_afirmar(d != NULL, "Diferencia(A, B) devuelve estructura válida");
    if (d) {
        ...
        for (int id = 1; id <= 5; ++id) {
            ...
            if (pa && !pb)
                pa2m_afirmar(tp1_buscar_id(d, id) != NULL, "Contiene id=%d (exclusivo de A)", id);
        }
...
void tp1_diferencia_dos_tp_vacio_devuelve_tp_vacio()
{
    ...
    pa2m_afirmar(d != NULL, "Diferencia de vacíos devuelve estructura válida");
    if (d) {
        pa2m_afirmar(tp1_cantidad(d) == 0, "Cantidad 0 en la diferencia vacía");
        tp1_destruir(d);
    }
...
void tp1_diferencia_largo_cantidad_correcta()
{
    ...
    pa2m_afirmar(d != NULL, "Diferencia(largo, normal) devuelve estructura válida");
    if (d) {
        pa2m_afirmar(tp1_cantidad(d) == esperado,
                     "Cantidad correcta en la diferencia largo+normal (%zu)", esperado);

        pa2m_afirmar(tp1_buscar_id(d, 30) != NULL || esperado==0, "Si es exclusivo de A, id=30 está en la diferencia");
        pa2m_afirmar(tp1_buscar_id(d, 2)  == NULL, "No está id=2 (solo en B)");
        pa2m_afirmar(tp1_buscar_id(d, 1) == NULL, "No está id=1 (está en ambos)");
...
void tp1_diferencia_primer_parametro_null_devuelve_null()
{
    ...
    pa2m_afirmar(u == NULL, "tp1_diferencia con primer parametro NULL devuelve NULL");
...
void tp1_diferencia_segundo_parametro_null_devuelve_null()
{
    ...
    pa2m_afirmar(u == NULL, "tp1_diferencia con segundo parametro NULL devuelve NULL");
...
void tp1_diferencia_ambos_parametros_null_devuelve_null()
{
    pa2m_afirmar(tp1_diferencia(NULL, NULL) == NULL,
                 "tp1_diferencia con ambos parámetros NULL devuelve NULL");
}
...
void tp1_buscar_nombre_nombre_correcto_encuentra_pokemon()
{
    ...
    pa2m_afirmar(p != NULL, "tp1_buscar_nombre encuentra pokemon correctamente");
    if (p) pa2m_afirmar(p->id == 52, "El id del pokemon encontrado es el esperado (52)");
...
void tp1_buscar_nombre_en_archivo_largo_encuentra_nombre_largo()
{
    ...
    pa2m_afirmar(p != NULL, "tp1_buscar_nombre busca por nombre largo encuentra pokemon correctamente");
    if (p) pa2m_afirmar(p->id == ref->id, "El id coincide con la referencia");
...
void tp1_buscar_nombre_no_altera_cantidad()
{
    ...
    pa2m_afirmar(antes == despues, "Buscar por nombre no altera la cantidad de pokemones");
...
void tp1_buscar_nombre_nombre_null_devuelve_null()
{
    ...
    pa2m_afirmar(p == NULL, "tp1_buscar_nombre con nombre NULL devuelve NULL");
...
{
    tp1_t *tp = tp1_leer_archivo(CSV_VACIO);
    pa2m_afirmar(tp != NULL, "Tp vacío creado correctamente");
    if (!tp) return;
...
void tp1_buscar_id_id_correcto_encuentra_pokemon()
{
    ...
    pa2m_afirmar(p != NULL, "tp1_buscar_id encuentra pokemon correctamente");
    if (p) pa2m_afirmar(strcmp(p->nombre, "Meowth") == 0,
...
void tp1_buscar_id_en_archivo_largo_encuentra_pokemon()
{
    ...
    pa2m_afirmar(p != NULL, "tp1_buscar_id encuentra pokemon en archivo largo");
...
void tp1_buscar_id_no_altera_cantidad()
{
    ...
    pa2m_afirmar(antes == despues, "Buscar por id no altera la cantidad de pokemones");
...
void tp1_cada_pokemon_recorre_en_orden_por_id()
{
    ...
    size_t cant = tp1_cantidad(tp);
    int ids[1024] = {0};
    struct recolector_ids r = { .ids = ids, .cantidad_ids = 0, .capacidad = 1024, .cortar_en = 0 };

    size_t aplicados = tp1_con_cada_pokemon(tp, recolectar_ids, &r);
    pa2m_afirmar(aplicados == cant, "Aplica la función a todos los pokemones");
...
void tp1_cada_pokemon_corte_temprano_funcion_false()
{
    ...
    int ids[8] = {0};
    struct recolector_ids r = { .ids = ids, .cantidad_ids = 0, .capacidad = 8, .cortar_en = 2 };

    size_t aplicados = tp1_con_cada_pokemon(tp, recolectar_ids, &r);
    pa2m_afirmar(aplicados == 2, "La función corta temprano cuando devuelve false");
    pa2m_afirmar(estan_ordenados_por_id(ids, r.cantidad_ids),
...
void tp1_cada_pokemon_cuenta_aplicados_igual_cantidad()
{
    ...
    pa2m_afirmar(aplicados == cant && iterados == cant,
...
{
    int iterados=0;
    size_t aplicados = tp1_con_cada_pokemon(NULL, contar_pokemons, &iterados);
    pa2m_afirmar(aplicados == 0, "tp1_cada_pokemon con tp NULL devuelve cero");
}
...
void tp1_cada_pokemon_funcion_null_devuelve_cero()
{
    ...
    pa2m_afirmar(aplicados == 0, "tp1_cada_pokemon con funcion NULL devuelve cero");
...
void tp1_cada_pokemon_tp_vacio_devuelve_cero()
{
    ...
    int iterados=0;
    size_t aplicados = tp1_con_cada_pokemon(tp, contar_pokemons, &iterados);
    pa2m_afirmar(aplicados == 0 && iterados == 0, "tp1_cada_pokemon devuelve cero y coincide con cantidad iterados");
