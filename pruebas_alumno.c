...
void tp1_leer_archivo_muchos_pokemones_cuenta_bien()
{
	...
	pa2m_afirmar(tp != NULL,
		     "CSV de muchas lineas crea la estructura válida");
	if (tp) {
...
void tp1_leer_archivo_mezcla_validos_invalidos()
{
	...
	if (tp) {
		pa2m_afirmar(tp1_cantidad(tp) == ESP_MEZCLA_VALIDOS,
			     "Cantidad de líneas válidas correctamente (%d)",
			     ESP_MEZCLA_VALIDOS);
		tp1_destruir(tp);
	}
}
...
void tp1_leer_archivo_todas_lineas_invalidas()
{
	...
	if (tp) {
		pa2m_afirmar(
			...
			"Cantidad 0 cuando todas las líneas son inválidas (%d)",
...
void tp1_leer_archivo_linea_final_sin_newline()
{
	...
	pa2m_afirmar(tp != NULL,
		     "Csv con ultima línea sin '\\n' se procesa correctamente");
	if (tp) {
		pa2m_afirmar(
			tp1_cantidad(tp) == ESP_SIN_NL_FINAL_CANT,
			"Cuenta correcta incluyendo la última línea sin newline (%d)",
...
void tp1_cantidad_tp_valido_devuelve_cantidad_correcta()
{
	...
	pa2m_afirmar(
		...
		"tp1_cantidad devuelve cantidad correcta para CSV válido (%d)",
		ESP_OK_CANTIDAD);
...
void tp1_cantidad_csv_mixto_cuenta_solo_validos()
{
	tp1_t *tp = tp1_leer_archivo(CSV_MEZCLA);
	pa2m_afirmar(tp != NULL,
		     "CSV mixto (válidos + inválidos) crea estructura no-NULL");
	if (!tp)
		return;

	size_t cant = tp1_cantidad(tp);
	pa2m_afirmar(cant == ESP_MEZCLA_VALIDOS,
		     "tp1_cantidad cuenta solo las líneas válidas (%d)",
		     ESP_MEZCLA_VALIDOS);
...
void tp1_cantidad_tp_vacio_devuelve_cero()
{
	tp1_t *tp = tp1_leer_archivo(CSV_VACIO);
	pa2m_afirmar(tp != NULL,
		     "Archivo existente pero vacío crea estructura no-NULL");
	if (!tp)
		return;
...
void tp1_union_mismo_tp_devuelve_otro_igual()
{
	...
	if (u) {
		...
		pa2m_afirmar(
			cant_u == cant_a,
			"La cantidad en la unión coincide con la original (%zu)",
			cant_a);
...
{
	tp1_t *largo =
		tp1_leer_archivo(CSV_LARGO); /* 15 elementos (tu lista) */
	...
	pa2m_afirmar(u != NULL,
		     "Unión(largo, normal) devuelve estructura válida");
	if (u) {
		pa2m_afirmar(tp1_cantidad(u) == 18,
			     "Cantidad correcta en la unión largo+normal (%d)",
			     18);
...
void tp1_union_unir_muchas_veces_sigue_contando_bien()
{
	...
	if (u4) {
		pa2m_afirmar(
			tp1_cantidad(u4) == 8,
			"Tras múltiples uniones, la cantidad sigue siendo %d",
			8);
...
void tp1_interseccion_mismo_tp_devuelve_otro_igual()
{
	...
	pa2m_afirmar(i != NULL,
		     "Interseccion(A, A) devuelve estructura válida");
	if (i) {
		size_t cant_i = tp1_cantidad(i);
		pa2m_afirmar(
			cant_i == cant_a,
			"La cantidad en la intersección coincide con la original (%zu)",
			cant_a);
		...
		if (p)
			pa2m_afirmar(tp1_buscar_id(i, 1) != NULL,
				     "id=1 está en la intersección");
		p = tp1_buscar_id(a, 2);
		if (p)
			pa2m_afirmar(tp1_buscar_id(i, 2) != NULL,
				     "id=2 está en la intersección");
		p = tp1_buscar_id(a, 3);
		if (p)
			pa2m_afirmar(tp1_buscar_id(i, 3) != NULL,
				     "id=3 está en la intersección");
		p = tp1_buscar_id(a, 4);
		if (p)
			pa2m_afirmar(tp1_buscar_id(i, 4) != NULL,
				     "id=4 está en la intersección");
		p = tp1_buscar_id(a, 5);
		if (p)
			pa2m_afirmar(tp1_buscar_id(i, 5) != NULL,
				     "id=5 está en la intersección");
...
void tp1_interseccion_tp_distintos_devuelve_otro_con_suma_correcta()
{
	...
	pa2m_afirmar(i != NULL,
		     "Interseccion(A, B) devuelve estructura válida");
	if (i) {
...
void tp1_interseccion_dos_tp_vacio_devuelve_tp_vacio()
{
	...
	pa2m_afirmar(i != NULL,
		     "Intersección de vacíos devuelve estructura válida");
	if (i) {
		pa2m_afirmar(tp1_cantidad(i) == 0,
			     "Cantidad 0 en la intersección vacía");
		tp1_destruir(i);
	}
...
void tp1_interseccion_largo_suma_correctamente()
{
	...
	pa2m_afirmar(i != NULL,
		     "Intersección(largo, normal) devuelve estructura válida");
	if (i) {
		pa2m_afirmar(
			tp1_cantidad(i) == esperado,
			"Cantidad correcta en la intersección largo+normal (%zu)",
			esperado);

		pa2m_afirmar(tp1_buscar_id(i, 30) == NULL,
			     "No está id=30 (solo en largo)");
		pa2m_afirmar(tp1_buscar_id(i, 2) == NULL,
			     "No está id=2 (solo en normal)");
		pa2m_afirmar(tp1_buscar_id(i, 1) != NULL || esperado == 0,
			     "Si comparten, id=17 está en la intersección");
...
void tp1_interseccion_unir_muchas_veces_sigue_contando_bien()
{
	...
	if (i3) {
		pa2m_afirmar(
			...
			"Tras múltiples intersecciones, la cantidad sigue siendo %zu",
			esperado);
...
void tp1_diferencia_dos_tp_vacio_devuelve_tp_vacio()
{
	...
	pa2m_afirmar(d != NULL,
		     "Diferencia de vacíos devuelve estructura válida");
	if (d) {
		pa2m_afirmar(tp1_cantidad(d) == 0,
			     "Cantidad 0 en la diferencia vacía");
		tp1_destruir(d);
	}
...
void tp1_diferencia_largo_cantidad_correcta()
{
	...
	pa2m_afirmar(d != NULL,
		     "Diferencia(largo, normal) devuelve estructura válida");
	if (d) {
		pa2m_afirmar(
		...
		pa2m_afirmar(
			...
			"Si es exclusivo de A, id=30 está en la diferencia");
		pa2m_afirmar(tp1_buscar_id(d, 2) == NULL,
			     "No está id=2 (solo en B)");
		pa2m_afirmar(tp1_buscar_id(d, 1) == NULL,
			     "No está id=1 (está en ambos)");
...
void tp1_cada_pokemon_recorre_en_orden_por_id()
{
	...
	pa2m_afirmar(aplicados == cant,
		     "Aplica la función a todos los pokemones");
...
void tp1_cada_pokemon_corte_temprano_funcion_false()
{
	...
	pa2m_afirmar(aplicados == 2,
		     "La función corta temprano cuando devuelve false");
	pa2m_afirmar(estan_ordenados_por_id(ids, r.cantidad_ids),
